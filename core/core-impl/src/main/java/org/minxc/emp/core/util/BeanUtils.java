...
public class BeanUtils {
     ...
     * @return
     */
    public static Object getValue(String columnType, QueryOperator queryOP, String valStr) {
        Object value = null;
    ...
    public static <T> List<T> listToTree(List<T> list) {
        ...
        if (!(list.get(0) instanceof TreeModel)) {
            throw new RuntimeException("树形转换出现异常。数据必须实现Tree接口！");
        }

        List<T> returnList = new ArrayList<T>();
        for (TreeModel tree : (List<TreeModel>) list) {
            tempMap.put(tree.getId(), tree);
        }

        for (TreeModel obj : (List<TreeModel>) list) {
